-- Álbum público por defecto + detalle de cruces sin exigir misma ciudad (alineado con Intercambio).

UPDATE public.user_profiles
SET
  privacy_settings = COALESCE(privacy_settings, '{}'::jsonb) || jsonb_build_object(
    'album_visibility',
    'public',
    'proposals_from',
    COALESCE(privacy_settings->>'proposals_from', 'anyone')
  )
WHERE
  privacy_settings IS NULL
  OR (privacy_settings->>'album_visibility') IS DISTINCT FROM 'public';

-- Fuente: scripts/supabase/0023-part2-overlap.sql (sin gate de misma ciudad).
...
COMMENT ON FUNCTION public.exchange_overlap_detail (uuid) IS
  'Cruces de intercambio: mismo album_edition; álbum del peer no privado; sin exigir misma ciudad.';
