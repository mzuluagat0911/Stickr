...
- **Tests:** `tests/unit/privacy-jitter.test.ts`; E2E `tests/e2e/profile.spec.ts` (requiere `E2E_*`). Vitest usa alias `@/` en `vitest.config.ts`.
...
- Marcado de álbum / catálogo y stats reales (`user_stickers`).
